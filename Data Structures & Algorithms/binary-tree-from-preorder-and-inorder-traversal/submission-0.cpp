...
public:
    TreeNode* build(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, unordered_map<int, int>& mp) {
        ...
        root->left = build(preorder, preStart+1, preStart+numsLeft, inorder, inStart, inIndex-1, mp);
        root->right = build(preorder, preStart+numsLeft+1, preEnd, inorder, inIndex+1, inEnd, mp);
        return root;
    }
